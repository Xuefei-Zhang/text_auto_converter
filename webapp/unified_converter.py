...
def parse_i2c_command(command_str):
    ...
    read_match = re.match(
        r"i2cread\s+(\d+)\s+(0x[0-9a-fA-F]+)\s+(0x[0-9a-fA-F]+)\s+(\d+)\s+(\d+)",
        command_str.strip(),
    )
    if read_match:
    ...
    write_match = re.match(
        r"i2cwrite\s+(\d+)\s+(0x[0-9a-fA-F]+)\s+(0x[0-9a-fA-F]+)\s+(\d+)\s+(\d+)\s+(.+)",
        command_str.strip(),
    )
    if write_match:
    ...
    alt_write_match = re.match(
        r"i2cwrite\s+(\d+)\s+(0x[0-9a-fA-F]+)\s+(0x[0-9a-fA-F]+)\s+(\d+)\s+(.+)",
        command_str.strip(),
    )
    if alt_write_match:
    ...
    alt_read_match = re.match(
        r"i2cread\s+(\d+)\s+(0x[0-9a-fA-F]+)\s+(0x[0-9a-fA-F]+)\s+(\d+)",
        command_str.strip(),
    )
    if alt_read_match:
...
def convert_txt_to_ini(txt_content):
    ...
    for line in lines:
        ...
        # Parse I2C commands
        cmd_type, bus, slave_addr, reg_addr, width, data_values = parse_i2c_command(
            line
        )
        ...
        if cmd_type == "read":
            ...
            if slave_addr != current_slave_addr or current_operation_mode == "write":
                ...
                slave_addr_8bit = hex(
                    slave_addr_7bit_int << 1
                )  # Shift left by 1, keep LSB as 0
            ...
            for i, data_val in enumerate(data_values):
                if i == 0:
                    ...
                    reg_hex = reg_addr[2:]  # Remove '0x' prefix
                    data_hex = (
                        f"{data_val:02x}" if data_val < 256 else f"{data_val:04x}"
                    )
...
def convert_ini_to_txt(ini_content):
    ...
    txt_lines.append(
        "#                       FreeRTOS I2C Commands for Sensing USB Card"
    )
    ...
    for line in lines:
        ...
        if line.startswith("#") or line.startswith(";"):
            ...
            if comment:  # Only add if there's actual comment text
                ...
                if (
                    "Configuration for Sensing USB Card" in comment
                    and "Converted from FreeRTOS format" in comment
                ):
        ...
        reg_match = re.match(
            r"REG=\s*(0x[0-9a-fA-F]+)(?:\s*,\s*(0x[0-9a-fA-F]+))?", line
        )
        if reg_match:
            ...
            if data_val is not None:
                ...
                if current_slave_addr:
                    ...
                    txt_lines.append(
                        f"i2cwrite {current_bus} {current_slave_addr} {reg_addr} 2 1 {data_val}"
                    )
                else:
                    # If no slave address has been set yet, use a default (7-bit)
                    txt_lines.append(
                        f"i2cwrite {current_bus} 0x36 {reg_addr} 2 1 {data_val}"
                    )  # 0x6C >> 1 = 0x36
            else:
                ...
                if current_slave_addr:
                    ...
                    txt_lines.append(
                        f"i2cread {current_bus} {current_slave_addr} {reg_addr} 2 1"
                    )
                else:
                    # If no slave address has been set yet, use a default (7-bit)
                    txt_lines.append(
                        f"i2cread {current_bus} 0x36 {reg_addr} 2 1"
                    )  # 0x6C >> 1 = 0x36
            continue
...
def parse_adi_fae_config(content):
    """
    Parse ADI FAE C++ configuration format (MAX9295+MAX9296)

    Format: 0x04,0x90,0x03,0x13,0x00, // comment
    - Byte 0: Prefix (usually 0x04)
    - Byte 1: I2C device address (7-bit, e.g., 0x80, 0x82, 0x90)
    - Byte 2-3: Register address (16-bit)
    - Byte 4: Register value

    Special cases:
    - Delay: 0x00,0x78, // 120 msec delay (0x78 = 120)

    Args:
        content (str): ADI FAE C++ file content
    ...
    device_map = {
        0x80: "MAX9295A (GMSL-A)",
        0x82: "MAX9295A (GMSL-B)",
        0x90: "MAX9296A (Deserializer)",
    }
    ...
    for line in lines:
        ...
        if len(parts) == 2 and parts[0] == "0x00":
            try:
                delay_val = int(parts[1], 16)
                operations.append(
                    {
        ...
        if len(parts) >= 5:
            try:
                ...
                device_addr = int(parts[1], 16)
                reg_high = int(parts[2], 16)
                reg_low = int(parts[3], 16)
                reg_value = int(parts[4], 16)

                if prefix != 0x04:
                    continue

                reg_addr = (reg_high << 8) | reg_low

                if device_addr != int(current_device_info["addr"], 16):
                    device_name = device_map.get(
                        device_addr, f"Unknown Device (0x{device_addr:02X})"
                    )
                    current_device_info = {
                        "name": device_name,
                        "addr": f"0x{device_addr:02X}",
                        "addr_8bit": f"0x{device_addr:02X}",
                        "comments": [],
                    }
                ...
                operations.append(
                    {
                        "device": current_device_info.copy(),
                        "reg_addr": f"0x{reg_addr:04X}",
                        "reg_data": f"0x{reg_value:02X}",
                        "op_type": "write",
                    }
                )
...
def convert_adi_fae_to_ini(adi_operations):
    """
    Convert ADI FAE format to INI format

    Args:
        adi_operations (list): List of parsed ADI FAE register operations
    ...
    ini_lines.append("#                       Configuration for Sensing USB Card")
    ini_lines.append("#                Converted from ADI FAE Format (MAX9295+MAX9296)")
    ini_lines.append(
        "# *************************************************************************"
    )
    ini_lines.append("#")
    ...
    for op in adi_operations:
        ...
        if current_slave_addr != op["device"]["addr_8bit"]:
            ...
            ini_lines.append("")
            ini_lines.append(
                f"# I2C Slave Address: {op['device']['addr']} ({op['device']['name']})"
            )
...
def convert_adi_fae_to_txt(adi_operations):
    """
    Convert ADI FAE format to FreeRTOS TXT format
    ...
    txt_lines.append("#                Converted from ADI FAE Format (MAX9295+MAX9296)")
    txt_lines.append(
        "# *************************************************************************"
    )
    txt_lines.append("")
    ...
    for op in adi_operations:
        ...
        if current_slave_addr != op["device"]["addr"]:
            ...
            addr_7bit_int = int(op["device"]["addr"], 16)
            addr_6bit = hex(addr_7bit_int >> 1)
            current_slave_addr_7bit = addr_6bit

        if op["op_type"] == "write":
            txt_lines.append(
                f"i2cwrite {current_bus} {current_slave_addr_7bit} {op['reg_addr']} 2 1 {op['reg_data']}"
            )
...
def parse_vendor_config(content):
    ...
    for line in lines:
        ...
        if line.startswith("#") and ("delay" in line.lower() or "wait" in line.lower()):
            ...
            if delay_val == 10 and (
                "delay here" in line.lower() or "had delay" in line.lower()
            ):
                delay_val = 10
            operations.append(
                {
        ...
        if line.startswith("#"):
            if "Device" in line:
                device_match = re.search(
                    r"Device \d+: ([^(]+) \([^)]+Addr: (0x[0-9a-fA-F]+)\)", line
                )
                if device_match:
                ...
                "max9296" in line.lower()
                or "max96717" in line.lower()
                or "ox08d10" in line.lower()
                or "ox03c" in line.lower()
            ):
                alt_device_match = re.search(
                    r"(max9296|max96717|ox08d10|ox03c|ox\d+d\d+?)\s+(0x[0-9a-fA-F]+)",
                    line.replace(":", " "),
                    re.IGNORECASE,
                )
                if alt_device_match:
                    ...
                    simple_match = re.search(
                        r"(max9296|max96717|ox08d10|ox03c|ox\d+d\d+?)\W+(0x[0-9a-fA-F]+)",
                        line,
                        re.IGNORECASE,
                    )
                    if simple_match:
...
def convert_vendor_to_txt(vendor_operations):
    ...
    for op in vendor_operations:
        ...
        if op["op_type"] == "write":
            ...
            # Format: i2cread bus slave_addr reg_addr width num_bytes
            txt_lines.append(
                f"i2cread {current_bus} {current_slave_addr_7bit} {op['reg_addr']} 2 1"
            )
        # For other operation types, do nothing
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified converter for sensor capture hardware configurations"
    )
    parser.add_argument(
        "--mode",
        choices=["vendor_to_both", "ini_to_txt", "txt_to_ini", "adi_fae_to_both"],
        required=True,
        help="Conversion mode: vendor_to_both, ini_to_txt, txt_to_ini, or adi_fae_to_both",
    )
    parser.add_argument("--input", required=True, help="Input file path")
    parser.add_argument(
        "--output", help="Output file path (for ini_to_txt or txt_to_ini)"
    )
    parser.add_argument(
        "--output_ini", help="Output INI file path (for vendor_to_both)"
    )
    parser.add_argument(
        "--output_txt", help="Output TXT file path (for vendor_to_both)"
    )
    ...
    if args.mode == "vendor_to_both":
        if not args.output_ini or not args.output_txt:
            print(
                "Error: For vendor_to_both mode, both --output_ini and --output_txt are required."
            )
            sys.exit(1)
    ...
    elif args.mode == "adi_fae_to_both":
        if not args.output_ini or not args.output_txt:
            print(
                "Error: For adi_fae_to_both mode, both --output_ini and --output_txt are required."
            )
            sys.exit(1)

        adi_operations = parse_adi_fae_config(input_content)

        ini_content = convert_adi_fae_to_ini(adi_operations)

        txt_content = convert_adi_fae_to_txt(adi_operations)
